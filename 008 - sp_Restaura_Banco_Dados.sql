...
Alterações:
	29/07/2022 - Ícaro: Ajuste na forma de obter o nome dos arquivos lógicos do backup a ser restaurado.
						O mesmo passou a ser obtido direto do arquivo .bak, através do restore filelistonly
...
(
	...
	Begin
		 Set NoCount On;
			...
			--ATUALIZANDO O NOME DOS ARQUIVOS LÓGICOS DO BACKUP A SER RESTAURADO
			BEGIN
				EXEC dbo.sp_OBTER_NOME_ARQUIVO_LOGICO_BACKUP @DIR_MIDIA_BACKUP, @SIGLA_CLIENTE
			END

			SET    @ARQ_LOGICO_DATA = (SELECT NOME_LOGICO_DADOS FROM PARAMETROS_RESTORE WHERE CLIENTE = @SIGLA_CLIENTE)
			SET	   @ARQ_LOGICO_LOG = (SELECT NOME_LOGICO_LOG FROM PARAMETROS_RESTORE WHERE CLIENTE = @SIGLA_CLIENTE)


			--CONCATENANDO OS DIRETÓRIOS
			SET @DIR_ARQ_DADOS = (SELECT @DIR_BANCO + @BD_CLIENTE_SEM_COLCHETE + '_Data.mdf')
			SET @DIR_ARQ_LOG = (SELECT @DIR_BANCO + @BD_CLIENTE_SEM_COLCHETE + '_Log.ldf')
			...
			BEGIN
				...
				BEGIN
					EXEC [REGISTRO_RESTORE_BD].DBO.sp_Cria_Registro_Backup @TIPO_BANCO, @SIGLA_CLIENTE, @DIR_MIDIA_BACKUP, @OBSERVACOES
				END
